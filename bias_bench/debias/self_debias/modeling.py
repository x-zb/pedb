...
class MaskedLMWrapper(ABC):
    ...
    def get_token_logits_self_debiasing(
        ...
        # print(input_ids.shape) # (1,480)
        # print(input_ids)
        
        # input_ids = input_ids[:, 1:] # comment out when evaluating perplexity
        # Remove leading [CLS] tokens, originally designed for SelfDebias, where input is [CLS]...[SEP] (note that [CLS] is added in prefixes)
        # For WikiText, individual seqs ([CLS]...[SEP]) are concatenated and truncated, so the firt token is not [CLS]
        # More importantly, the first token may be [MASK] when we prepare the pseudo perplexity inputs,
        # in this case there will be no [MASK] token after the remove, which will lead to errors in `mask_positions` and `lm_logits[mask_positions]`  
        ...
        mask_positions = input_ids_repeated == self._tokenizer.mask_token_id # (2,seq_len)
        ...
        lm_logits = outputs["logits"] # (2,seq_len,vocab_size)
        ...
        # print(lm_logits[mask_positions].shape)
        return lm_logits[mask_positions][0] # lm_logits[mask_positions]~(2,vocab_size)

    def get_token_logits_self_debiasing_for_lama(
        self,
        input_ids: torch.Tensor,
        debiasing_prefixes: List[str],
        decay_constant: float = 50,
        ...
        return lm_logits # (bsz=1,pre_len+seq_len,vocab_size)
    ...
    def compute_loss_self_debiasing(
        ...
        relevant_labels = labels[input_ids == self._tokenizer.mask_token_id] # (bsz,seq_len)->(bsz)
