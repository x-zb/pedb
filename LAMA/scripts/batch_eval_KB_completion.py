...
def run_thread(arguments):
    ...
    sample_MRR, sample_P, experiment_result, return_msg = metrics.get_ranking(
        arguments["filtered_log_probs"], # (bsz,seq_len,vocab_size)
...
def lowercase_samples(samples, use_negated_probes=False):
    ...
    for sample in samples:
        ...
        sent_list = sample["masked_sentences"] if "masked_sentences" in sample else [] # only for T-REx, which has templates
        for sentence in sent_list:
            sentence = sentence.lower()
            sentence = sentence.replace(base.MASK.lower(), base.MASK)
            lower_masked_sentences.append(sentence)
        sample["masked_sentences"] = lower_masked_sentences # this is only effective for ConceptNet and SQuAD, since GoogleRE and T-REx's masked sentences are generated by templates
...
def filter_samples(model, samples, vocab_subset, max_sentence_length, template):
    ...
    for sample in samples:
        ...
        if "obj_label" in sample and "sub_label" in sample:
            ...
            if obj_label_ids:
                recostructed_word = " ".join( # this will not add space on single words
                    [model.vocab[x] for x in obj_label_ids]
                ).strip() # strip will not remove the starting space (288) in GPT2's vocab, a usual space is 32
            else:
            ...
            if excluded:
                ...
                samples_exluded += 1
            elif not recostructed_word or recostructed_word not in [sample["obj_label"],chr(288)+sample["obj_label"]]:
                msg += "\tEXCLUDED object label {} not in model vocabulary\n".format(
                    sample["obj_label"]
                )
                samples_exluded += 1
...
def main(args, shuffle_data=True, model=None):
    ...
    if model_type_name == "fairseq":
        ...
        model_name = model_type_name.title() # GPT2
    ...
    if args.common_vocab_filename is not None:
        ...
        model.optimize_top_layer(vocab_subset) # nothing to do with bert or gpt2

        filter_logprob_indices, index_list = model.init_indices_for_filter_logprobs(
            vocab_subset, logger
        ) # the indices of vocab_subset's words in the model's tokenizer's vocab 
    ...
    # this is effective for GoogleRE and T-REx (which have templates), not for ConceptNet and SQuAD
    # Each template contains a `[X]` and `[Y]`, while other parts are all lowercased words. 
    # Since the sub_label and obj_label are all lowercased in lowercase_samples(), the generated probe will be fully lowercased.
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        (
            original_log_probs_list, # (bsz,seq_len,vocab_size)
            token_ids_list, # list (len=bsz) of lists of token ids (padded)
            masked_indices_list, # list (len=bsz) of lists of masked token position
        ...
        if vocab_subset is not None:
            ...
            filtered_log_probs_list = original_log_probs_list # (bsz,seq_len,vocab_size)

        label_index_list = []
        for sample in samples_b:
            obj_label_id = model.get_id(sample["obj_label"]) # the id in model's vocab
            ...
            elif model.vocab[obj_label_id[0]] not in [sample["obj_label"],chr(288)+sample["obj_label"]]:
                raise ValueError(
                    "object label {} not in model vocabulary".format(
                        sample["obj_label"]
                    )
        ...
        arguments = [
            {
                "original_log_probs": original_log_probs, # (seq_len,vocab_size)
                "filtered_log_probs": filtered_log_probs, # (seq_len,vocab_size)
                "token_ids": token_ids, # list (len=seq_len)
                "vocab": model.vocab,
                "label_index": label_index[0], # obj token's id in model.vocab
                "masked_indices": masked_indices, # list of mask token position
                "interactive": args.interactive,
                "index_list": index_list, # the indices of vocab_subset's words in the model's tokenizer's vocab
                "sample": sample,
            }
        ...
        for idx, result in enumerate(res): # (experiment_result, sample_MRR, sample_P, sample_perplexity, msg)
            ...
            element["sample_Precision"] = sample_P # 'P_AT_X'
    ...
    # Precision
    Precision /= len(list_of_results) # P_AT_X (default=10)
