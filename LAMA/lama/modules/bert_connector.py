...
class Bert(Base_Connector):
    ...
    def __init__(self, args, tokenizer, model, _is_self_debias=False, bias_type=None, vocab_subset = None):
        '''args is only used once in line 70'''
        super().__init__()
        ...
        do_lower_case = False
        if 'uncased' in args.bert_model_name: # bert_model_name:
            do_lower_case=True

        # Load pre-trained model tokenizer (vocabulary)
        self.tokenizer = tokenizer # BertTokenizer.from_pretrained(dict_file)
        ...
        self.masked_bert_model = model # BertForMaskedLM.from_pretrained(bert_model_name)

        # self.masked_bert_model.eval()

        # ... to get hidden states
        self.bert_model = self.masked_bert_model.bert if hasattr(self.masked_bert_model,'bert') else self.masked_bert_model._model.bert
    ...
    def __get_input_tensors_batch(self, sentences_list):
        ...
        for sentences in sentences_list:
            ...
            # (1,seq_len),(1,seq_len),list of [MASK] idx,list of tokens
        ...
        # (bsz,seq_len), (bsz,seq_len), (bsz,seq_len), list (len=bsz) of lists, list (len=bsz) of lists
    ...
    def _cuda(self):
        if not self._is_self_debias:
            ...
            self.masked_bert_model._model.cuda()
    ...
    def get_batch_generation(self, sentences_list, logger= None, 
        ...
        with torch.no_grad():
            if self._is_self_debias:
                ...
                logits = self.masked_bert_model.get_token_logits_self_debiasing_for_lama(
                    tokens_tensor.to(self._model_device),
                    debiasing_prefixes=debiasing_prefixes,
                    decay_constant=50,
                    epsilon=0.01,
                ) # (bsz,pre_len+seq_len,vocab_size)
                ...
                logits = logits[:, bias_type_to_position[self._bias_type] :, :] # (bsz,seq_len,vocab_size)
            else:
                logits = self.masked_bert_model(
                    input_ids=tokens_tensor.to(self._model_device),
                    token_type_ids=segments_tensor.to(self._model_device),
                    attention_mask=attention_mask_tensor.to(self._model_device),
                )[0] # no labels are provided, so the first output is not loss; it's logits of size # (bsz,seq_len,vocab_size)

            log_probs = F.log_softmax(logits, dim=-1).cpu() # (bsz,seq_len,vocab_size)
        ...
        # (bsz,seq_len,vocab_size), list (len=bsz) of lists, list (len=bsz) of lists
