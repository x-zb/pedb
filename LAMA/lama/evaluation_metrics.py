...
def __max_probs_values_indices(masked_indices, log_probs, topk=1000): # list,(seq_len,vocab_size)

    # score only first mask
    masked_indices = masked_indices[:1]

    masked_index = masked_indices[0] # int
    log_probs = log_probs[masked_index] # (vocab_size)

    value_max_probs, index_max_probs = torch.topk(input=log_probs,k=topk,dim=0)
    index_max_probs = index_max_probs.numpy().astype(int) # (topk)
    value_max_probs = value_max_probs.detach().numpy() # (topk)

    return log_probs, index_max_probs, value_max_probs
    # (vocab_size), (topk), (topk) 
...
def __print_top_k(value_max_probs, index_max_probs, vocab, mask_topk, index_list, max_printouts = 10):
    ...
    for i in range(mask_topk):
        filtered_idx = index_max_probs[i].item() # id in model.vocab

        if index_list is not None:
            # the softmax layer has been filtered using the vocab_subset ??? where is it?
...
def get_ranking(log_probs, masked_indices, vocab, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True):
    ...
    experiment_result['topk'] = result_masked_topk # [{i=0~1000,idx,log_prob,word}], results
    ...
    if label_index is not None:
        ...
        ranking_position = (index_max_probs==query).nonzero() # (topk).nonzero = [[rank_idx]] is (1,1)
        ...
        label_perplexity = log_probs.gather(
            dim=0,
            index=tokens,
        ) # equivalent to log_probs[label_index]
        PERPLEXITY = label_perplexity.item()

        if len(ranking_position) >0 and ranking_position[0].shape[0] != 0: # label_index is in the topk index_max_probs
